...
layer.open = function(options){
  ...
  if($('#layerOpen').length == 0){
    $('<div id="layerOpen" class="layermbox layermbox2" index="0"><div class="laymshade"></div><div class="layermmain"><div class="section"><div class="layermchild  layermanim"><div class="layermcont"><i></i><i class="laymloadtwo"></i><i></i><div id="layerCont"></div></div></div></div></div></div>').appendTo('body');
  };
  var _time = o.time *1000;
  clearTimeout(layerFlag);
  if(o.type == 2){
    $('#layerOpen').addClass('layermbox2');
  }else{
    $('#layerOpen').removeClass('layermbox2');
    layerFlag = setTimeout(function(){
      $('#layerOpen').hide()
    },_time)
  }
  $('#layerOpen').show();
  $('#layerCont').text(o.content);
}
layer.closeAll = function(){
  $('#layerOpen').hide()
}
//点击放大图片
var isEvent = true,_picListArr = [];
var showLargePicture = function(obj,container){
  if($('#pictureView').length == 0){
     $('<div id="pictureView" class="pictureView"><div class="pageInfo"></div><span class="closePicView"></span><div class="picViewMain"><img></div></div>').appendTo('body');
  };
  var _imgSrc = $(obj).attr('src');
  if(_imgSrc){
    _imgSrc = _imgSrc.split('-smaller');
    _imgSrc = _imgSrc[0]+_imgSrc[1];
    $('#pictureView').find('img:eq(0)').attr('src',_imgSrc);
    
    $('#pictureView').fadeIn(100,function(){
      setTimeout(function(){
    ...
    if(container){
      var _src = null;
      _picListArr = $(container).find('img').map(function(){
        _src = $(this).attr('src');
        _src = _src.split('-smaller');
        _src = _src[0]+_src[1];
        return _src;
      });
      var _info = ($.inArray(_imgSrc,_picListArr)+1) +" / "+_picListArr.length
      $('#pictureView .pageInfo').text(_info);
    };
  }
  if(isEvent){
    eventPicture(container);
    isEvent = false
  }
}
var hidePictureView = function(){
  $('#pictureView').fadeOut(100,function(){
    $("#pictureView").removeClass('show');
  });
  _picListArr = [];
}
var eventPicture = function(){
  if(isEvent){
    //敲击某个元素时触发
    $("#pictureView").on("click",function(){
      if($("#pictureView").hasClass('show')){
        hidePictureView();
      }
    });
    $("#pictureView").on("swipeleft",function(){
      var len = _picListArr.length,nowSrc = $(this).find('img').attr('src');
      var aN = $.inArray(nowSrc,_picListArr);
      if(aN + 1 < len){
        $(this).find('img').attr('src',_picListArr[aN + 1]);
        $('#pictureView .pageInfo').text((aN + 2)+" / "+len);
      }
    });
    $("#pictureView").on("swiperight",function(){
      var len = _picListArr.length,nowSrc = $(this).find('img').attr('src');
      var aN = $.inArray(nowSrc,_picListArr);
      if(aN - 1 >= 0){
        $(this).find('img').attr('src',_picListArr[aN - 1]);
        $('#pictureView .pageInfo').text(aN+" / "+len);
      }
    });
  }
}
...
//登陆弹窗
var goToLogin = function(url,title){
  var _tit = '登陆后即可继续操作';
  if($('#goToLogin').length == 0){
     $('<div id="goToLogin" class="goToLogin"><div class="goToLoginCell"><div class="goToLoginContainer"><div class="pageInfoContent"></div><a href="" class="greenBtn">使用微信快捷登陆</a><span class="closeLogin"></span></div></div></div>').appendTo('body');
  };
  if(title){_tit = title};
  $('#goToLogin').fadeIn();
  $('#goToLogin .pageInfoContent').text(_tit);
  $('#goToLogin .greenBtn').attr('href',url);
  $('#goToLogin .greenBtn').unbind('click').bind('click',function(){$('#goToLogin').fadeOut()});
  $('#goToLogin .closeLogin').unbind('click').bind('click',function(){$('#goToLogin').fadeOut()})
}
...
//星星打分功能
var starScore = function(container,inputVal){
  $(container).empty();
  var startH = $(container).height(),
      startW = startH/2,mgR = ($(container).width() - startW*10)/4,_dom ='',sR = 0;
  $(container).css('position','relative');
  for(var i = 0; i < 10; i++){
    _dom += '<div class="startTarget" data-idx="'+(i+1)+'" style="width:'+startW+'px;height:'+startH+'px;position:absolute;top:0;left:'+sR+'px;z-index:111;"></div>';
    if(i!= 0 && (i+1)%2 == 0 ){
      sR += (startW+mgR);
    }else{
      sR += startW
  ...
  $(container).find('.startTarget').unbind('click').bind('click',function(){
    var _idx = parseInt($(this).attr('data-idx'));
    $(container).attr('data-star',_idx/2);
    if(inputVal){
      $(inputVal).val(_idx/2);
    }
  })
